...
<body>
    ...
    <div class="espace_travail">
        <div class="boite-reponse" style="width: 100%; height: 50px; background-color: rgb(204, 255, 204); color: darkgreen; display: flex;  align-items: center; font-size: 20px; font-weight: bold;">
            <i class="fa-solid fa-circle-check" style="margin-left:10px;"></i> Dossier transféré vers la DEP avec succès !
        </div>
        ...
        <table>
            ...
            <tbody>
                @foreach ($dossiers_externes as $d)
                    <tr>
                        <td>{{$d->id}}</td>
                        <td>{{$d->intitule}}</td>
                        <td>{{$d->envoyer_par}}</td>
                        <td>{{$d->dateEnvoiDossier}}</td>
                        <td>
                            <ul>
                                <a href="/transfert_dep/{{$d->id}}" onclick="event.preventDefault();if(confirm('voulez vous transferer ce dossier a la DEP ?'){window.location.href='/transfert_dep/{{$d->id}}';})"><li><i class="fa-solid fa-share"></i>Transférer à la DEP</li></a>
                            </ul>
                        </td>
                    </tr>
                @endforeach
