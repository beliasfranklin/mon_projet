    <!-- espace de travail du membre CPP -->
    @if ($user->id==$users_roles->id_user && $users_roles->id_role==22)
       <div class="espace_travail">
            <div class="conteneurs-stats-users">
                <div>
                    Nombre de dossiers à traiter (CPP)
                    <span>
                        @php
                            $nb_dossiers_cpp = $dossiers_cpp->count();
                            echo $nb_dossiers_cpp;
                        @endphp
                    </span>
                </div>
            </div>
            <span class="users-cpp">Dossiers à traiter par la CPP</span>
    ...
    <!-- espace de travail du membre CEI -->
    @if ($user->id==$users_roles->id_user && $users_roles->id_role==23)
       <div class="espace_travail">
            <div class="conteneurs-stats-users">
                <div>
                    Nombre de dossiers à traiter (CEI)
                    <span>
                        @php
                            $nb_dossiers_cei = $dossiers_cei->count();
                            echo $nb_dossiers_cei;
                        @endphp
                    </span>
                </div>
            </div>
            <span class="users-cei">Dossiers à traiter par la CEI</span>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
...
<body>
    ...
    <div class="conteneur_menus" id="sideMenu">
        <div class="conteneur_titre_logo_minsante">
            ...
            @endif
            <!-- navbar du Ministre -->
            @if($user->id==$users_roles->id_user && $users_roles->id_role==19)
            ...
            @if($user->id==$users_roles->id_user && $users_roles->id_role==18)
                ...
                <ul class="menus">
                    ...
                    <li><a href="/consulter_résultats"><i class="fa-solid fa-bell"></i>Notifications</a></li>
                    <li><a href="/se deconnecter/{{$user->id}}" onclick="seDeconnecter(event)"><span style="color: #ff0000;"><i class="fa-solid fa-right-from-bracket" style="color: #ff0000;"></i>Se deconnecter</span></a></li>
                </ul>
            @endif
            <!-- nav bar du membre du courrier de la DEP-->
            @if ($user->id==$users_roles->id_user && $users_roles->id_role==20)
                <span style="font-size: 14px;">Courrier DEP</span>
                <ul class="menus">
                    <li><a href="/dashboard_dep"><i class="fa-solid fa-gauge"></i>Tableau de bord</a></li>
                    <li><a href="/courrier_dep_voir_dossier_externe"><i class="fa-solid fa-inbox"></i>Gerer dossiers externes</a></li>
                    <li><a href="/se deconnecter/{{$user->id}}" onclick="seDeconnecter(event)"><span style="color: #ff0000;"><i class="fa-solid fa-right-from-bracket" style="color: #ff0000;"></i>Se deconnecter</span></a></li>
                </ul>
            @endif
        </div>    
    ...
    @php
        ...
        //dossiers internes envoyés par le ministre
        $nb_total_dossiers_internes_envoyes=Dossier::where('typeDossier','interne')->count();
        $dossiers_internes=Dossier::where('typeDossier','interne')->get();
        //les dossiers pour la cpp
        $dossiers_cpp=Dossier::where('statutDossier','coteCPP')->get();
        //les dossiers pour la cei
        $dossiers_cei=Dossier::where('statutDossier','coteCEI')->get();
    ...
    @endif
    <!-- espace de travail du sécretariat de la DEP-->
    @if ($user->id==$users_roles->id_user && $users_roles->id_role==21)
       <div class="espace_travail">
            <div class="conteneurs-stats-users">
                <div>
                    Nombres de dossiers Côtés à la CPP
                    <span>
                        @php
                            $nb_dossiers_cotes_cpp=Dossier::where('statutDossier','coteCPP')->count();
                            echo $nb_dossiers_cotes_cpp;
                        @endphp
                    </span>
                </div>
            </div>
            <div class="conteneurs-stats-users">
                <div>
                    Nombres de dossiers Côtés à la CEI
                    <span>
                        @php
                            $nb_dossiers_cotes_cpp=Dossier::where('statutDossier','coteCEI')->count();
                            echo $nb_dossiers_cotes_cpp;
                        @endphp
                    </span>
                </div>
            </div>
            <span class="users-ext">Situation des dossiers pour la structure CPP</span>
            <div class="table-container">
                <table>
                    <thead>
                        <tr>
