...
<body>
    ...
    <div class="espace_travail">
        <span class="envoi_dossier_interne"><span class="dossier_interne">Détails du dossier envoyé à la DEP</span></span><br><br><br>
            ...
            <a href="/transfert_dep/{{$d->id}}" onclick="event.preventDefault();if(confirm('voulez vous transferer ce dossier a la DEP ?'){window.location.href='/transfert_dep/{{$d->id}}';})"><button class="bouton-validation">Envoyer Dossier à la DEP</button></a><br><br>
            <button class="bouton-validation" onclick="decrireMotifRejet()">Rejeter le dossier</button><br><br>
            <div id="contenu_rejet_dossier"></div>
    </div>
    <script>
        ...
        function decrireMotifRejet()(){
            const a=document.getElementById('contenu_rejet_dossier');
            a.innerHTML='<span class="a">Motif du rejet:<br></span><form method="post" action="/traitement_rejet_dossier/{{$dossier->id}}">@csrf<textarea name="texte"cols="70" rows="10" placeholder="motif du rejet"></textarea><br><br><input type="submit" class="bouton-validation"></form>';
        }
    </script>
</body>
</html>