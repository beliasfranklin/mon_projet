...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Formulaire */
        .formulaire_modifier_utilisateur {
            background: white;
            padding: 30px;
            border-radius: 8px;
        ...
        /* Styles pour les valeurs actuelles */
        .info-section {
            background: #f8f9fa;
            padding: 15px;
            border-radius: 8px;
...
<body>
    ...
    <div class="espace_travail">
        <div class="utilisateurs">
            ...
            <span class="users">Modifier Utilisateur {{$user1->name}}</span>
        </div>
        
        <!-- Section d'information actuelle -->
        <div class="info-section">
            <div class="info-title">Informations actuelles de l'utilisateur</div>
            <div class="info-content">
                <div class="info-item">
                    <div class="info-label">Nom</div>
                    <div class="info-value">{{$user1->name}}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Email</div>
                    <div class="info-value">{{$user1->email}}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Téléphone</div>
                    <div class="info-value">{{$tel[0]['numeroTelephone']}}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Rôle actuel</div>
                    <div class="info-value">{{$nom_role_user[0]['nomRole']}}</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Structure actuelle</div>
                    <div class="info-value">{{$nom_structure_user[0]['nomStructure']}}</div>
                </div>
            </div>
        </div>
        
        <div class="formulaire_modifier_utilisateur">
            <form action="/traiter_formulaire_modification_utilisateur/{{$user1->id}}" method="post" id="modificationForm">
                @csrf
                
                <div class="form-group">
                    <span class="a">Nom complet</span>
                    <i class="fa-solid fa-user"></i>
                    <input type="text" name="nom" id="nom" placeholder="Nom complet" required value="{{$user1->name}}">
                </div>

                <div class="form-group">
                    <span class="a">Email</span>
                    <i class="fa-solid fa-envelope"></i>
                    <input type="email" name="email" id="email" placeholder="Email" value="{{$user1->email}}">
                </div>

                <div class="form-group">
                    <span class="a">Numéro de Téléphone (Pour Whatsapp de préférence)</span>
                    <i class="fa-solid fa-phone"></i>
                    <input type="tel" name="tel" id="tel" placeholder="Numéro de téléphone" value="{{$tel[0]['numeroTelephone']}}">
                </div>

                <div class="form-group">
                    <span class="a">Mot de passe (laisser vide pour ne pas modifier)</span>
                    <i class="fa-solid fa-lock"></i>
                    <input type="password" name="mdp" id="mdp" placeholder="Nouveau mot de passe">
                </div>

                <div class="form-group">
                    <span class="a">Rôle</span>
                    <div class="current-value">
                        <strong>Valeur actuelle :</strong> {{$nom_role_user[0]['nomRole']}}
                    </div>
                    <select name="role" id="role">
                        <option value="{{$nom_role_user[0]['nomRole']}}">{{$nom_role_user[0]['nomRole']}} (actuel)</option>
                ...
                <div class="form-group">
                    ...
                    <div class="current-value">
                        <strong>Valeur actuelle :</strong> {{$nom_structure_user[0]['nomStructure']}}
                    </div>
                    <select name="structure" id="structure">
                        <option value="{{$nom_structure_user[0]['nomStructure']}}">{{$nom_structure_user[0]['nomStructure']}} (actuelle)</option>
    ...
    <script>
        ...
        document.getElementById('modificationForm').addEventListener('submit', function(e) {
            ...
            if (!telRegex.test(tel.replace(/\s/g, ''))) {
                e.preventDefault();
                alert('Veuillez entrer un numéro de téléphone valide (7-15 chiffres, peut commencer par +).');
                return false;
            }
        });
        ...
        // Message d'information pour le mot de passe
        document.addEventListener('DOMContentLoaded', function() {
            const mdpInput = document.getElementById('mdp');
            mdpInput.addEventListener('focus', function() {
                if (!this.getAttribute('data-info-shown')) {
                    this.setAttribute('placeholder', 'Laisser vide pour garder l\'actuel');
                    this.setAttribute('data-info-shown', 'true');
                }
            });
        });
    </script>
