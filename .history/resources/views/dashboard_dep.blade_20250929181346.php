...
<body>
    ...
    <div class="conteneur_menus">
        <div class="conteneur_titre_logo_minsante">
            <img src="logo-minante.png" alt="logo-minsante" class="logo-minsante"><br>
                            ...
                            <tr>
                                ...
                                <td>
                                    @php
                                        ...
                                        ->join('user_roles','roles.id','=','user_roles.id_role')
                                        ->where('user_roles.id_user',$u->id)
                                        ->select('roles.nomRole')
                                        ->first();
                                    @endphp
                                    {{$role_user->nomRole}}
                                </td>
                                <td>
                                    @php
                                        $statut_connexion=DB::table('statut_connexions')
                                        ->where('id_user',$u->id)
                                        ->select('estConnecte')
                                        ->first();
                                    @endphp
                                    @if($statut_connexion && $statut_connexion->estConnecte==1)
                                        Connecté
                                    @else
                                        Déconnecté
                                    @endif
                                ...
                                <td>
                                    <ul>
                                        <a href="/admin_voir_details_utilisateur/{{$u->id}}"><li><i class="fa-solid fa-eyes"></i>voir détails</li></a>
                                        <a href="/admin_modifier_role_utilisateur/{{$u->id}}"><li><i class="fa-solid fa-user-pen"></i>modifier rôle</li></a>
                                        <a href="/admin_bloquer_utilisateur/{{$u->id}}" onclick="return confirm('Voulez vous vraiment bloquer cet utilisateur ?')"><li><i class="fa-solid fa-user-lock"></i>bloquer</li></a>
                                        <a href="/admin_debloquer_utilisateur/{{$u->id}}" onclick="return confirm('Voulez vous vraiment débloquer cet utilisateur ?')"><li><i class="fa-solid fa-user-check"></i>débloquer</li></a>
                                    </ul>
                                </td>
                            </tr> 
                        @endforeach
