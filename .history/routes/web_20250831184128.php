...
Route::get("/supprimer_utilisateur/{id}",function($id){
    ...
    if($user_id){
        ...
        $conn=StatutConnexion::where('id_user',$user_id);
        $conn->update(['estConnecte'=> 1]);
        return view('admin_reponse_supprimerutilisateur',compact('users_roles','user','all_users','all_user_role','all_telephone','all_roles','all_structures','all_users_structures'));
    }else{
        return view('connexion');
    }
    DB::delete("delete from users where id=?",[$id]);
