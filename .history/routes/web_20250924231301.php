// Sécurise la route POST sans ID pour éviter l'erreur de méthode
Route::post('/reinitialiser_mot_de_passe', function() {
    return redirect('/reinitialisation_mdp/error/0');
});
<?php
...
Route::post('/reinitialiser_mot_de_passe/{id}',[MonController::class,'reinitialiserMotDePasse']);
...
Route::get('/test_result/{email}',function($email){
    echo $email;
});
...
Route::post('/traitement_rejet_dossier',function($id){
    $user_id = session('user_id'); //id de session de l'utilisateur
    $user=User::find($user_id); //recherche le membre courrier de la DEP
    if($user_id){
        $conn=StatutConnexion::where('id_user',$user_id);
