...
Route::post('/traiter_formulaire_modification_utilisateur/{id}',function(Request $request,$id){
    ...
    if($user_id){
        ...
        $action=new Action();
        $action->nomAction="l'utilisateur ".$user->name."a modifié les informations de l'utilisateur ".$user1->name;
...
Route::get('/modifier_utilisateur/{id}',function($id){
    ...
    $structure_user=UsersStructures::where('user_id',34)->get();  //on prend le numéro de la structure du user
    ...
    if($user_id){
        $action=new Action();
        $action->nomAction="l'utilisateur ".$user->name."veut modifier l'utilisateur ".$user1->name;
        $action->id_user=$user->id;
        $action->save();
        $conn=StatutConnexion::where('id_user',$user_id);
        $conn->update(['estConnecte'=> 1]);
...
Route::get("/supprimer_utilisateur/{id}",function($id){
    ...
    if($user_id){
        ...
        $all_users_structures=UsersStructures::all();  //les structures auxquelles sont rattachés chaque utilisateurs
        $action=new Action();
        $action->nomAction="l'administrateur a supprimé l'utilisateur ".$user1->name;
        $action->id_user=$user->id;
        $action->save();
        $conn=StatutConnexion::where('id_user',$user_id);
        $conn->update(['estConnecte'=> 1]);
        return view('admin_reponse_supprimer utilisateur_utilisateur',compact('users_roles','user','all_users','all_user_role','all_telephone','all_roles','all_structures','all_users_structures'));
    }else{
        return view('connexion');
    }
...
Route::get("/se deconnecter",function(Request $request){
    $request->session()->forget('user_id');
    echo $request->session()->forget('user_id');
