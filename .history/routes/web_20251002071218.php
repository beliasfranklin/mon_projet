// Dashboard CPP
Route::get('/cpp_dashboard', [MonController::class, 'dashboardCpp'])->name('cpp.dashboard');
Route::get('/cpp_voir_dossier/{id}', [MonController::class, 'voirDossierCpp'])->name('cpp.voir_dossier');

// Dashboard CEI
Route::get('/cei_dashboard', [MonController::class, 'dashboardCei'])->name('cei.dashboard');
Route::get('/cei_voir_dossier/{id}', [MonController::class, 'voirDossierCei'])->name('cei.voir_dossier');
<?php
...
// ...existing code...
Route::post('/traiter_formulaire_modification_utilisateur/{id}', [MonController::class, 'traiterFormulaireModificationUtilisateur']);
// ...existing code...
...
//return $request->file('fichiers');
    ...
    foreach($request->file('fichiers') as $fichier){
        ...
        echo $val."<br>";
        $fichier->storeAs('Ministre',$fichier->getClientOriginalName()); 
...
Route::get('/ministre_envoyer_dossier_interne_ok',function(){
    ...
    if($user_id){
        ...
        $statut_user=StatutUser::where('id_user',$user_id)->get()->first();
        $statut_user->update(['statut'=>'disponible']);
        return view('Ministre.ministre_envoyer_dossier_interne_response',compact('user'));
        //return view('admin_reponse_supprimer_utilisateur',compact('users_roles','user','all_users','all_user_role','all_telephone','all_roles','all_structures','all_users_structures'));
    }else{
        return redirect('/');
    }
...
//dans le cas ou le Ministre n'envoie pas le dossier interne
Route::get('/ministre_envoyer_dossier_interne_pas_ok',function(){
    $user_id = session('user_id'); //id de session de l'utilisateur
    $user=User::find($user_id); //recherche le ministre
    if($user_id){
        $conn=StatutConnexion::where('id_user',$user_id);
        ...
        $statut_user=StatutUser::where('id_user',$user_id)->get()->first();
        $statut_user->update(['statut'=>'disponible']);
        return view('Ministre.ministre_envoyer_dossier_interne_response_pas_ok',compact('user'));
        //return view('admin_reponse_supprimer_utilisateur',compact('users_roles','user','all_users','all_user_role','all_telephone','all_roles','all_structures','all_users_structures'));
    }else{
        return redirect('/');
    }
...
Route::get('/usager_externe_envoyer_dossier_externe_ok',function(){
    ...
    if($user_id){
        ...
        $statut_user=StatutUser::where('id_user',$user_id)->get()->first();
        $statut_user->update(['statut'=>'disponible']);
        return view('UsagerExterne.usager_externe_envoyer_dossier_externe_response',compact('user'));
    }else{
        return redirect('/');
    }
});
Route::get('/usager_externe_envoyer_dossier_externe_pas_ok',function(){
    $user_id = session('user_id'); //id de session de l'utilisateur
    $user=User::find($user_id); //recherche l'utilisateur externe
    if($user_id){
        $conn=StatutConnexion::where('id_user',$user_id);
        ...
        $statut_user=StatutUser::where('id_user',$user_id)->get()->first();
        $statut_user->update(['statut'=>'disponible']);
        return view('UsagerExterne.usager_externe_envoyer_dossier_externe_response_pas_ok',compact('user'));
    }else{
        return redirect('/');
    }
});
