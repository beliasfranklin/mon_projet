    ...
    public function traiterFormulaireDossierInterne(Request $request){
        ...
        if($user_id){
            ...
            if($membre_connecte){
                ...
                //historiser l'action
                $action->nomAction="le ministre a envoyé un dossier interne intitulé ".$dossier->intitule." à la dep et à été recu par ".$membre_connecte->name;
                $action->id_user=$user->id;
                return redirect('/ministre_envoyer_dossier_interne_ok');

            }else{
                $action->nomAction="le ministre n'a pas pu envoyé un dossier interne intitulé ".$dossier->intitule." à la dep car aucun membre du sécrétariat de la DEP disponible";
                $action->id_user=$user->id;
    ...
    public function traiterFormulaireDossierExterne(Request $request){
        ...
        if($user_id){
            ...
            if($membre_connecte){
                ...
                //$membre_connecte->notify(new NotificationNouveauDossierInterne($dossier));
                //historiser l'action
                $action->nomAction="l'utilisateur".$user->name."a envoyé un dossier intitulé".$dossier->intitule." au courrier de la DEP et à été recu par ".$membre_connecte->name;
                $action->id_user=$user->id;
                return redirect('/usager_externe_envoyer_dossier_externe_ok');

            }else{
                $action->nomAction="l'utilisateur".$user->name."n'a pas pu envoyer un dossier intitulé".$dossier->intitule." au courrier de la DEP et à été recu par car aucun membre disponible";
                $action->id_user=$user->id;
