...
class MonController extends Controller
{
    ...
    public function verifierCode(Request $request){
        ...
        $c=$request->code;
        $user=User::where('email',$e)->first();
        $code_trouve=CodesVerifications::where('id_user',$user->id)->where('code',$c)->first();
        if($code_trouve){
            //le code est correct
            //supprimer le code de la table
            CodesVerifications::where('id_user',$user->id)->where('code',$c)->delete();
            return redirect('/reinitialisation_mdp/user/'.$user->id);
        }else{
            return redirect('/recevoir_code/error/'.$e);
        }
