...
class MonController extends Controller
{
    ...
    public function creerCompteUtilisateurExterne(Request $request){
        $validated = $request->validate(
            ...
            [
                ...
                'confirm_password.required' => 'Le mot de passe est requis.',
                'confirm_password.min' => 'Le mot de passe doit contenir au moins 8 caractères.',
                'confirm_password.max' => 'Le mot de passe ne doit pas dépasser 255 caractères.',
                'confirm_password.confirmed' => 'La confirmation du mot de passe ne correspond pas.',
                'confirm_password.regex' => 'Le mot de passe doit contenir au moins une minuscule, une majuscule et un chiffre.'
            ]
        );
        ...
        $user_roles->id_role = 15; // rôle par défaut pour utilisateur externe
    ...
    public function envoyerCode(Request $request){
        ...
        if(!User::where('email',$e)->exists()){
            ...
            return view('recevoir_code',compact('e'));
        }
    ...
    public function verifierCode(Request $request){
        ...
        $c=$request->code;
        $user=User::where('email',$e)->first();
        $code_trouve=CodesVerifications::where('code',$c)->first();
        if($code_trouve){
            //le code est correct
            //supprimer le code de la table
            CodesVerifications::where('code',$c)->delete();
            return redirect('/reinitialisation_mdp/user/{{$user->id}}');
        }else{
            return redirect('/recevoir_code/error');
        }
    ...
    public function reinitialiserMotDePasse(Request $request){
        $request->validate(
            ...
            [
                ...
                'password.confirmed' => 'La confirmation du mot de passe ne correspond pas.',
                'password.regex' => 'Le mot de passe doit contenir au moins une minuscule, une majuscule et un chiffre.',
                'password_confirm.required' => 'Le mot de passe est requis.',
                'password_confirm.min' => 'Le mot de passe doit contenir au moins 8 caractères.',
                'password_confirm.max' => 'Le mot de passe ne doit pas dépasser 255 caractères.',
                'password_confirm.confirmed' => 'La confirmation du mot de passe ne correspond pas.',
                'password_confirm.regex' => 'Le mot de passe doit contenir au moins une minuscule, une majuscule et un chiffre.'
            ]
        );
