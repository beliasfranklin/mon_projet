...
class MonController extends Controller
{
    ...
    {
        $admin = $this->getAdminOrRedirect();
        if (!$admin) {
            return redirect('/')->with('error', "Session administrateur introuvable.");
        }
        $this->updateAdminStatus($admin->id);
        $users_roles = UserRole::where('id_user', $admin->id)->first();
        $users = User::all();
        ...
        $action = new Action();
        $action->nomAction = "L'administrateur {$admin->name} gère les utilisateurs";
        $action->id_user = $admin->id;
        $action->save();
        return view('admin.admin_gerer_utilisateurs', compact('admin', 'users_roles', 'roles', 'structures', 'users_structures', 'telephone', 'user_role', 'users'));
    }
    ...
    public function creerCompte()
    {
        ...
        $roles = Roles::all();
        $action = new Action();
        $action->nomAction = "L'administrateur {$admin->name} veut créer un compte utilisateur";
        $action->id_user = $admin->id;
        $action->save();
        return view('admin.admin_creer_utilisateur', compact('admin', 'users_roles', 'roles', 'structure'));
    }
...
public function traiterFormulaireCreationCompte(Request $request)
{
    $validated = $request->validate([
        'nom' => [
            'required', 'string', 'max:255', "regex:/^[a-zA-ZÀ-ÿ\s\-']+$/u"
        ],
        'email' => [
        ...
        'mdp' => [
            'required', 'min:8', 'max:255', 'regex:/^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).{8,}$/'
        ],
        'role' => [
            'required', 'exists:roles,nomRole'
        ],
        'structure' => [
            'required', 'exists:structures,nomStructure'
        ]
    ], [
        'nom.required' => 'Le nom est requis.',
        'nom.string' => 'Le nom doit être une chaîne de caractères.',
    ...
    try {
        ...
        UsersStructures::create([
            'id_user' => $user->id,
            'id_structure' => $structure ? $structure->id : null
        ]);
        ...
        \DB::commit();
        return redirect('/creation_utilisateurs_ok')->with('success', 'Utilisateur créé avec succès.');
    } catch (\Exception $e) {
        \DB::rollBack();
...
public function afficherReponseModificationUtilisateur()
{
    ...
    $roles = Roles::all();
    $action = new Action();
    $action->nomAction = "L'administrateur {$admin->name} consulte la réponse de modification de compte utilisateur";
    $action->id_user = $admin->id;
    $action->save();
    return view('admin.admin_reponse_modification_utilisateur', compact('admin', 'users_roles', 'roles', 'structure'));
}
...
public function afficherReponseCreationUtilisateur()
{
    ...
    $roles = Roles::all();
    $action = new Action();
    $action->nomAction = "L'administrateur {$admin->name} consulte la réponse de création de compte utilisateur";
    $action->id_user = $admin->id;
    $action->save();
    return view('admin.admin_reponse_creation_utilisateur', compact('admin', 'users_roles', 'roles', 'structure'));
}
...
public function rechercherUtilisateur(Request $request){
    ...
    public function envoyerDossierInterne(){
        ...
        $action=new Action();
        if($user_id){
            $conn=StatutConnexion::where('id_user',$user_id)->first();
            $conn->update(['estConnecte'=> 1]);
            $statut_user=StatutUser::where('id_user',$user_id)->get()->first();
            $statut_user->update(['statut'=>'disponible']);
            ...
            $action->id_user=$user->id;
            $action->save();
            return view('Ministre.ministre_envoyer_dossier_interne',compact('user'));
        }else{
            return redirect('/');
        }
    }
    ...
    public function traiterFormulaireDossierInterne(Request $request){
        ...
        if($user_id){
            ...
            if($membre_connecte){
                ...
                foreach($docs as $d){
                    $d->storeAs($user->name,$d->getClientOriginalName(),'public');
                    PiecesJointes::create([
                        'nom'=>$d->getClientOriginalName(),
                        'chemin'=>$d->storeAs('Ministre/'.$dossier->intitule,$d->getClientOriginalName(),'public'),
                ...
                //historiser l'action
                $action->nomAction="le ministre a envoyé un dossier interne intitulé ".$request->nom." à la dep et à été recu par ".$membre_connecte->name;
                $action->id_user=$user->id;
                return redirect('/ministre_envoyer_dossier_interne_ok');

            }else{
                $action->nomAction="le ministre n'a pas pu envoyé un dossier interne intitulé ".$request->nom." à la dep car aucun membre du sécrétariat de la DEP disponible";
                $action->id_user=$user->id;
                return redirect('/ministre_envoyer_dossier_interne_pas_ok');
            }
    ...
    public function envoyerDossierExterne(){
        ...
        if($user_id){
            ...
            $action->id_user=$user->id;
            $action->save();
            return view('UsagerExterne.usager_externe_envoyer_dossier_externe',compact('user'));
        }else{
            return redirect('/');
        }
    }
    ...
    public function traiterFormulaireDossierExterne(Request $request){
        ...
        if($user_id){
            ...
            $request->validate(
                ...
                [
                'files.*.required'=>'Veuillez joindre au moins un document.',
                'files.*.file'=>'Chaque élément doit être un fichier valide.',
                'files.*.max'=>'Chaque fichier ne doit pas dépasser 100 Mo.',
                'nom.required'=>'Le nom du dossier est requis.',
                'nom.string'=>'Le nom du dossier doit être une chaîne de caractères.',
                'nom.max'=>'Le nom du dossier ne doit pas dépasser 255 caractères.'

                ]
            );
            //recherche d'un membre du courrier de la DEP
            $membres_CourrierDEP=UserRole::where('id_role',20)->get();
            $membre_connecte=null;
            $all_statut_users=StatutUser::all();
            foreach($membres_CourrierDEP as $m){
                foreach($all_statut_users as $s){
                    if($m->id_user==$s->id_user && $s->statut=='disponible'){
                        $membre_connecte=User::find($m->id_user);
                        break 2;
            ...
            if($membre_connecte){
                ...
                //enregistrment des pieces jointes en base de données et dans le storage
                foreach($docs as $d){
                    $d->storeAs($user->name,$d->getClientOriginalName(),'public');
                    PiecesJointes::create([
                        'nom'=>$d->getClientOriginalName(),
                        'chemin'=>$d->storeAs($user->name.'/'.$dossier->intitule,$d->getClientOriginalName(),'public'),
                        'taille'=>$d->getSize()/1024,
                        'idDossier'=>$dossier->id
                    ]);
                }
                ...
                //$membre_connecte->notify(new NotificationNouveauDossierInterne($dossier));
                //historiser l'action
                $action->nomAction="l'utilisateur".$user->name."a envoyé un dossier intitulé".$request->nom." au courrier de la DEP et à été recu par ".$membre_connecte->name;
                $action->id_user=$user->id;
                $dossier->save();
                return redirect('/usager_externe_envoyer_dossier_externe_ok');

            }else{
                $action->nomAction="l'utilisateur".$user->name."n'a pas pu envoyer un dossier intitulé".$request->nom." au courrier de la DEP et à été recu par car aucun membre disponible";
                $action->id_user=$user->id;
                return redirect('/usager_externe_envoyer_dossier_externe_pas_ok');
            }
...
// Traitement du formulaire de modification d'utilisateur
    public function traiterFormulaireModificationUtilisateur(Request $request, $id)
    {
        $admin = $this->getAdminOrRedirect();
        if (!$admin) {
            return redirect('/')->with('error', "Session administrateur introuvable.");
        ...
        try {
            ...
            $numero = NumeroTelephone::where('id_user', $user->id)->first();
            if ($numero) {
                $numero->numero = $validated['telephone'];
                $numero->save();
            }

            $userRole = UserRole::where('id_user', $user->id)->first();
            if ($userRole) {
                $userRole->id_role = $validated['role'];
                $userRole->save();
            }

            $userStructure = UsersStructures::where('id_user', $user->id)->first();
            if ($userStructure) {
                $userStructure->id_structure = $validated['structure'];
                $userStructure->save();
            }

            $action = new Action();
            $action->nomAction = "L'administrateur {$admin->name} a modifié le compte de {$user->name}";
            $action->id_user = $admin->id;
            $action->save();

            \DB::commit();
            return redirect()->route('admin.gerer_utilisateurs')->with('success', 'Utilisateur modifié avec succès.');
        } catch (\Exception $e) {
            \DB::rollBack();
            return back()->withInput()->withErrors(['error' => "Erreur lors de la modification de l'utilisateur : " . $e->getMessage()]);
        }
    }
    // Méthode privée pour vérifier la session admin
    private function getAdminOrRedirect()
    {
        $user_id = session('user_id');
        if (!$user_id) {
            return null;
    ...
    // Méthode privée pour mettre à jour le statut et la connexion
    private function updateAdminStatus($user_id)
    {
        StatutConnexion::where('id_user', $user_id)->update(['estConnecte' => 1]);
        StatutUser::where('id_user', $user_id)->update(['statut' => 'disponible']);
    }
