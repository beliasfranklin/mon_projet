...
public function afficherReponseModificationUtilisateur()
{
    ...
    return view(
        'admin_reponse_modifier_utilisateur',
        compact(
            'user',
            'users_roles',
            'roles',
...
// ...existing code...
    ...
    public function rechercherUtilisateur(Request $request){
        ...
        $roles=Roles::all();  //tout les roles
        $structures=Structure::all();  //toutes les structures
        $users_structures=UsersStructures::all();  //les structures auxquelles sont r
        $statut_user=StatutUser::all();
        if($user_id){
            $conn=StatutConnexion::where('id_user',$user_id);
            $conn->update(['estConnecte'=> 1]);
            $statut_user=StatutUser::where('id_user',$user_id)->get
            ()->first();
            $statut_user->update(['statut'=>'disponible']);
            $recherche=$request->recherche;
            $action->nomAction="l'utilisateur ".$user->name."a recherché un utilisateur
            dans la liste des utilisateurs";
            $action->id_user=$user->id;
            $action->save();
            return view('admin.admin_resultat_recherche_utilisateur',compact('user','users_roles','roles','structures','users_structures','telephone','user_role','users','recherche','statut_user'));
        }else{
            return redirect('/');
        }
    }
