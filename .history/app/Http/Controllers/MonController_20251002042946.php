public function afficherResultatHistorique(Request $request)
{
    ...
    // Filtres date_opt (période rapide)
    if ($request->filled('date_opt')) {
        $now = now();
        switch ($request->date_opt) {
            case "Aujourd'hui":
    ...
    // Filtres date et heure personnalisées
    if ($request->filled('date_historiques')) {
        $date = $request->date_historiques;
        if ($request->filled('heure_historiques')) {
            $heure = $request->heure_historiques;
            $query->where('created_at', '>=', $date.' '.$heure);
        } else {
            $query->whereDate('created_at', $date);
        }
    }

    // Filtres avancés traçabilité
    ...
    if ($request->filled('action_type')) {
        ...
        if ($type === 'connexion') {
            $query->where('nomAction', 'like', '%connexion%');
        } elseif ($type === 'modification') {
            $query->where('nomAction', 'like', '%modifi%');
        } elseif ($type === 'suppression') {
            $query->where('nomAction', 'like', '%supprim%');
        } elseif ($type === 'creation') {
            $query->where('nomAction', 'like', '%créé%')->orWhere('nomAction', 'like', '%création%');
        } elseif ($type === 'autre') {
            $query->where(function($q){
                $q->where('nomAction', 'not like', '%connexion%')
                  ->where('nomAction', 'not like', '%modifi%')
                  ->where('nomAction', 'not like', '%supprim%')
                  ->where('nomAction', 'not like', '%créé%')
                  ->where('nomAction', 'not like', '%création%');
            });
        }
    }
    if ($request->filled('periode_debut') && $request->filled('periode_fin')) {
        $query->whereBetween('created_at', [$request->periode_debut, $request->periode_fin]);
    }

    $hist = $query->orderBy('created_at', 'desc')->get();
    $users_hist = User::all();

    return view('admin.admin_resultat_historiques', compact('user', 'users_roles', 'hist', 'users_hist'));
}
<?php
